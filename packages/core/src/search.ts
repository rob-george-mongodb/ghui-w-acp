...
export interface ChangedFileSearchResult<F extends NamedFile = NamedFile> {
	readonly file: F
	readonly index: number
	readonly matchIndexes: readonly number[]
}

export const filterLabels = <L extends Labeled>(labels: readonly L[], query: string): readonly L[] => {
...
export const fuzzyPathMatch = (path: string, query: string): { readonly score: number; readonly matchIndexes: readonly number[] } | null => {
...
export const filterChangedFiles = <F extends NamedFile>(files: readonly F[], query: string): readonly ChangedFileSearchResult<F>[] => {
	const hasQuery = pathSearchTokens(query).length > 0
	const results: Array<ChangedFileSearchResult<F> & { readonly score: number }> = []
